...

{
  ...
  "summary": "〈記事テキスト〉を読み、(1)提携の背景と目的 (2)各社の役割分担 (3)提携が目指す具体的な成果 の３点を漏れなく網羅しつつ、一切の主観を排したニュートラルな事実の要約を550～650字で記述してください。",
  "synergyAnalysis": "楽観的な戦略家として、〈記事テキスト〉を基に「販売網の拡大」「技術の補完関係」「ブランド価値の向上」といった観点から、この提携が生み出すであろう具体的なシナジー効果を多角的に分析し、説得力のある論考を550～650字で記述してください。",
  "riskAnalysis": "慎重なリスク管理担当として、〈記事テキスト〉から「提携解消リスク」「実行計画の曖昧さ」「企業文化の衝突可能性」といった実行上の課題や潜在的リスクを最低３つ抽出し、品位を保ちつつも、具体的な対策案を添えて冷静に指摘してください。550～650字でまとめてください。",
  "futureOutlook": "シニアパートナーの視点から、〈記事テキスト〉を基に、この提携の(1)短期的な業界へのインパクト (2)3〜5年後の中期的な展開 (3)将来的なM&Aへの発展可能性の３つの時間軸で予測してください。その際、競合他社の追随可能性や類似・先行事例を踏まえた分析を含めてください。全体を550～650字で論じてください。"
}`;
...

{
  ...
  "summary": "〈記事テキスト〉を読み、(1)事業モデル (2)資金使途 (3)市場環境 の３点を漏れなく網羅しつつ、一切の主観を排したニュートラルな事実の要約を550～650字で記述してください。固有名詞や数字は、可能な限り初出時のみ使用を心掛けてください。",
  "positiveAnalysis": "強気のVCパートナーとして、〈記事テキスト〉を基に「成長ドライバー」「競争優位」「マイルストーン達成確度」の観点から、強気の投資仮説を展開してください。他の投資家がまだ気づいていないような、斬新な示唆を提示し、読者が思わず膝を打つような説得力のある論考を550～650字で記述してください。",
  "negativeAnalysis": "辛口で知られるトップVCとして、〈記事テキスト〉から「事業計画の穴」「対処困難な競合の脅威」「資金繰りの潜在的懸念」を最低３つ抽出し、品位を保ちつつも愛のある、痛烈なダメ出しをしてください。単なる批判ではなく、事業の成功を願うからこその厳しい指摘として、断定的な論調で550～650字にまとめてください。誹謗中傷は絶対に禁止です。",
  "exitOutlook": "投資委員会の視点から、〈記事テキスト〉を基に、IPOと戦略的M&A（具体的な買い手候補を最低2社挙げること）の２つのシナリオを比較検討してください。それぞれの蓋然性、想定しうる時価総額水準のレンジ（悲観的な場合から楽観的な場合）を考慮し、最終的にどのようなExitが資金調達会社や投資家にとって魅力的かを結論づけて、550～650字で論じてください。"
}`;
      ...
      cors(req, res, async () => {
        ...
        try {
          ...
          if (extractedText.length < 100) {
           ...
           // ★★★ 変更点: モデルを gemini-1.5-pro-latest にアップグレード ★★★
          const model = genAI.getGenerativeModel({model: "gemini-1.5-pro-latest"});
          const fullPrompt = `${promptTemplate}\n\n--- 記事テキスト ---\n${extractedText.substring(0, 1000000)}`; // 1.5 Proは最大1Mトークンまで対応
